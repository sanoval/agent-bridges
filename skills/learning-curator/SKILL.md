...
`delegation-pipeline`, this skill is never pasted into a QA/Security Codex
prompt. It runs **after** a unit is done, not as part of
...
- **Codex:** still out of scope — Codex has no per-task skill loader
  ...
  QA/Security Codex prompt if Claude Code pastes its body in, same
  workaround as a project skill, just sourced from the global store
...
content, tool output, and delegated-agent (`antigravity`/Codex)
